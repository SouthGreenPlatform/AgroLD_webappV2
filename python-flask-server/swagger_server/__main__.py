...
def main():
    ...
    app.add_api('swagger.yaml', arguments={'title': 'AgroLD API'})
    app.run(port=8080)
app1 = Flask(__name__, template_folder='templates')
#SET SECRETKEY ---------------------------------------------------------------------------------
class Config_SK(object):
    app1.config['SECRET_KEY'] = '6efc92e4fdea016b2111bd8a6432f19b'
...
class Config_DB(object):
    ...
    app1.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://%(user)s:%(pw)s@%(host)s:%(port)s/%(db)s' % POSTGRES
    app1.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False # silence the deprecation warning

app1.config.from_object(Config_DB)
db = SQLAlchemy(app1)
...
@app1.route('/documentation')
def documentation():
    return render_template('documentation.html', title ='Documentation')

@app1.route('/quicksearch')
def quicksearch():
    return render_template('quicksearch.html', title ='Quick Search')


@app1.route('/search_advance/<tag>')
def advance(tag):
...
@app1.route('/test/sparql', methods = ['GET'])
def sparql():
...
@app1.route('/sparql1', methods = ['GET'])
def sparql1():
